...
kernel void spatialFilter(
    ...
    // Accepted samples are not necessarily converged samples: indirect indoor
    // lighting can retain isolated, high-variance paths after thirty frames.
    // Give only the nearest wavelet pass a variance-dependent strength on a
    // well-supported, rough ordinary surface. Glass/reflection/reactive tags,
    // fine geometry, contrasting material boundaries and wider passes retain
    // their existing safeguards. No filtered result enters temporal history.
    if (step==1 && fract(world.w)<0.1f && albedo.w>=0.3f && totalMoment>=6.0f) {
        float relativeDeviation=sqrt(variance)/max(mean,0.0005f);
        float residualNoise=smoothstep(0.1f,0.5f,relativeDeviation);
        float confidence=smoothstep(8.0f,16.0f,historyFrames);
        // A real shadow/reflection stripe has support along at least one axis.
        // Boost only a residual isolated in all four directions, with
        // compatible geometry/albedo on both sides. This leaves even a thin
        // diagonal illumination feature on an otherwise featureless plane.
        constexpr int2 axes[4]={int2(1,0),int2(0,1),int2(1,1),int2(1,-1)};
        float isolation=1.0f;
        for(uint axis=0;axis<4;++axis) {
            float pairLuma=0;
            for(int side=-1;side<=1;side+=2) {
                int2 q=int2(tid)+axes[axis]*side;
                if(!validPixel(q,u.sizeFlags.xy)) { isolation=0;continue; }
                uint2 pixel=uint2(q);
                if(spatialGeometryWeight(world,normalDepth,currentWorld.read(pixel),currentNormalDepth.read(pixel),footprint,1.0f)<0.5f
                    || surfaceColorWeight(albedo,currentAlbedo.read(pixel))<0.8f) isolation=0;
                pairLuma += reconstructionLuminance(finiteRadiance(inputHDR.read(pixel).rgb))*0.5f;
            }
            isolation=min(isolation,abs(centerLuma-pairLuma)/max(mean,0.0005f));
        }
        float isolatedResidual=smoothstep(0.02f,0.15f,isolation);
        strength=max(strength,0.75f*residualNoise*confidence*isolatedResidual*smoothstep(0.25f,0.38f,albedo.w));
    }
