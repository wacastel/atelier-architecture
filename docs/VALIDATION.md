...
Robie House adds seven two-minute architectural studies and a six-minute McCormick Place connection. Its Roman brick, limestone, cantilevered roofs, patterned transmitting glass and selected furnished interiors follow the consulted plans and restoration photographs. The south-lakefront corridor adds 14,478 mapped building polygons, 13,520 paths, 319 piers and 963 representative harbor boats. These remain referenced reconstructions and representative occupancy, with interpreted context façades and planting. [House references and scope](ROBIE-HOUSE.md) · [Map and aerial review](HYDE-PARK.md).

Chicago Demo now starts at a random Chicago location and view, then follows the established order through all six destinations. Selecting a Chicago location or view keeps the demo playing. Previous/next navigation crosses destination boundaries; wrapping between Robie House’s last view and Willis Tower’s first changes day/night. The 48 routes total 86 minutes 36 seconds at 1×. Paris is excluded. [Controls](DEMO.md).

The house passes 1,026 CPU geometry checks with 438,350 triangles, 190 art-glass panels and 38 lights. The corridor passes 46,350 data checks and has 4,919,716 triangles. Full-city playback passes 79,253 checks across all 57 routes, including Paris, with no blocked, unsupported or near-surface samples. Those route checks preceded two tiny exterior light fixtures; the exact ordered navigation triangle coordinates are byte-identical after that polish. The new Robie focus probe and three existing landmark probes hit their intended actual city geometry. [House](validation/v1.9/robie-geometry.json) · [Corridor data](validation/v1.9/hyde-park-data-validation.json) · [Corridor geometry](validation/v1.9/hyde-park-geometry.json) · [Navigation equivalence](validation/v1.9/robie-night-polish-navigation-equivalence.json) · [Playback](validation/v1.9/playback-validation.txt) · [Focus](validation/v1.9/focus-integration.json).

## Version 1.9 capacity and motion corrections

The first expanded-city images exposed a rendering defect beyond the 4 GiB vertex-data boundary: only the first portion of Robie’s grounds appeared, and stray geometry also affected McCormick’s water. Widening shader vertex indices alone did not fix it. Bounded static geometry descriptors restored the house and water; each hit’s geometry-local primitive ID is converted to the shared global vertex/material index, while the traffic instance retains its separate offset. This is an observed compatibility failure on the tested device, not a claimed published Apple buffer-size limit. [Discovery](validation/v1.9/geometry-capacity-discovery.json) · [Mapping review](validation/v1.9/geometry-partition-review.json).

The larger lighting bounds also exceeded the previous dense light-index cell budget. The budget is now 1,048,576 cells, with the same 64-metre cells, finite light support, ordered candidates and index cap. The range array is bounded at 8 MiB. Built-in Chicago self-tests now require both day and night indices to remain enabled. The CPU light-grid fixture passes 43,916 queries, and an independent GPU fixture verifies identical indexed/unindexed lighting pixels, including a changed-order negative control. [Light index](validation/v1.9/light-grid-validation.txt) · [Lighting GPU check](validation/v1.9/indexed-lighting-validation.txt).

The bounded large-geometry GPU fixture passes **77 checks**, using the production descriptor offsets and shader intersection helpers. It submits only eight real triangles in each of four separated sections plus one traffic triangle, and verifies static/traffic identity, global IDs, vertex positions and material reads beyond 4 GiB. It does not traverse all 45 million city triangles or every descriptor end. The original full-span synthetic diagnostics remain preserved: the mostly zero-area fixture failed, and the distributed 44.7-million-triangle build was terminated after approximately 205 seconds without reaching its probes. Their causes are not claimed resolved by the smaller fixture. Representative complete-city images independently verify that the house and McCormick water are restored. [Bounded result](validation/v1.9/large-geometry-gpu.json) · [Original failures and scope](validation/v1.9/large-geometry-fixture-investigation.json).

## Version 1.9 motion measurements

Repeated bilinear history reprojection blurred same-material ground shadows during the fast Hyde Park flight. A screen-displacement bound now shortens temporal history during faster movement while retaining long history for slow inspection. The preserved first run regressed from raw image RMSE 0.02251 to 0.02536 and from raw temporal residual 0.03198 to 0.03266. Actual-kernel synthetic tests isolate the cap against an otherwise identical uncapped shader: shadow-edge error improves 18.4% and 27.3% at 1.37 and 3.37 pixels/frame; a noiseless sequence improves 25.3%. Existing denoiser checks also pass. [Original failure](validation/v1.9/motion-before-fix.json) · [Denoiser GPU log](validation/v1.9/denoiser-validation.txt) · [Method and limitations](MOTION.md).

All **12 actual-scene cases in the final serial batch pass**: all seven Robie motion cases, plus Field Hall, Willis at night, Cloud Gate at night, Paris ironwork and the Eiffel night silhouette. Each uses **640 × 400, 32 frames, eight current samples and independent 128-sample references**. Measurements use the second half of each sequence. Raw and reconstructed display-space RGB presentations share one trace accumulation; temporal residual is the consecutive error difference after subtracting reference motion. Both error measures must improve, with the existing acceptance criteria retained.

| Moving case | Image RMSE: raw → reconstructed | Temporal residual: raw → reconstructed | Temporal reduction |
| --- | ---: | ---: | ---: |
| robie-cantilevers | 0.026275 → 0.017144 | 0.038906 → 0.016503 | 57.58% |
| robie-roman-brick | 0.049473 → 0.037137 | 0.071287 → 0.036274 | 49.11% |
| robie-art-glass | 0.063409 → 0.042902 | 0.090001 → 0.055154 | 38.72% |
| robie-living-room | 0.060971 → 0.044020 | 0.086317 → 0.055424 | 35.79% |
| robie-dining-night | 0.067866 → 0.046263 | 0.095807 → 0.058108 | 39.35% |
| robie-exterior-night | 0.016919 → 0.010725 | 0.024412 → 0.014233 | 41.70% |
| hyde-park-flight | 0.022534 → 0.021585 | 0.032048 → 0.028960 | 9.64% |
| field-hall | 0.073585 → 0.036258 | 0.104459 → 0.047568 | 54.46% |
| willis-night | 0.027667 → 0.027597 | 0.039546 → 0.039328 | 0.55% |
| cloud-gate-night | 0.037571 → 0.034160 | 0.052923 → 0.044418 | 16.07% |
| iron | 0.055118 → 0.032812 | 0.080651 → 0.031970 | 60.36% |
| night-silhouette | 0.042156 → 0.041105 | 0.058439 → 0.056737 | 2.91% |

[Complete summary](validation/v1.9/motion-final-summary.json) · [Robie and flight](validation/v1.9/motion-final-robie.json) · [Field Hall](validation/v1.9/motion-final-field.json) · [Willis](validation/v1.9/motion-final-willis.json) · [Cloud Gate](validation/v1.9/motion-final-bean.json) · [Paris iron](validation/v1.9/motion-final-iron.json) · [Paris night](validation/v1.9/motion-final-eiffel-night.json).

The twelve-case GPU batch and first standalone self-tests use executable SHA-256 `8e51d32907905508d118050910cd7893487a3ac17ea5b4f2404e2cf7d23636e9`. The final rebuild below adds the About-panel geography credit and a scoped CLI activity token. Exact source comparison confirms that model, route, renderer and shader inputs remain byte-identical; these earlier measurements retain their original executable scope. The final package has separate standalone checks and a shorter Field Hall activity experiment. [Production input equivalence](validation/v1.9/production-input-equivalence.json).

These reductions compare reconstruction with paired raw input, not with the previous release. The final complete-batch flight result is 4.21% lower image error and 9.64% lower temporal error; the earlier accepted isolated run, with its slightly different values, remains separately recorded. The Willis and Eiffel night improvements are modest. This run samples five earlier motion cases rather than repeating the entire historical suite, and does not certify every route, camera speed or quality setting as noiseless. [Earlier isolated flight](validation/v1.9/motion-flight-final.json) · [Final command record and executable/shader hashes](validation/v1.9/final-gpu-batch.json) · [Verbatim copied evidence](validation/v1.9/final-gpu-evidence-index.json).

**The final matched scene comparison measures a 2.606% increase for the expanded city on one Field Hall trace.** An unmodified version 1.8 build from commit `cdf45f9e01ed17b0a85ebde74cd3ecd368ea8088`, a version 1.9 build using Robie House before the small night-lighting polish, and the final production build ran serially under the same current Mac state after movie work finished. All use **640 × 400, sixteen frames, eight current samples and 64-sample references**, and all pass image/temporal quality checks.

| Matched Field Hall build | Mean paired GPU cost | Initial pipeline/AS setup |
| --- | ---: | ---: |
| Unmodified 1.8 baseline | 96.343 ms | 3.55 s |
| 1.9 before Robie night-lighting polish | 87.369 ms | 4.86 s |
| Final 1.9 production | 98.854 ms | 3.08 s |

This single matched trace supports retaining the continuous expanded city: the final build is about **2.6% higher** than the unchanged 1.8 baseline here. It does not establish a universal city-wide performance guarantee or statistically isolate the small lighting polish. These paired command times include tracing, reconstruction and both presentations; they are not native preview frame time or FPS. [Exact comparison](validation/v1.9/scene-performance/comparison.json) · [Baseline build provenance](validation/v1.9/scene-performance/baseline-build.json) · [Pre-polish build provenance](validation/v1.9/scene-performance/pre-polish-build.json) · [Copied logs, metrics and hashes](validation/v1.9/scene-performance/evidence-index.json).

The earlier separate reports remain preserved: **158.06 ms** for the final Field Hall case versus an earlier **39.62 ms** baseline. Their roughly fourfold difference is not reproduced in the matched comparison; even the unmodified 1.8 build does not reproduce its earlier faster timing under the current conditions. The older reports use 32 frames and 128-sample references, while every build in the matched comparison uses the same shorter 16-frame/64-reference setup. The cause of the timing variation remains unresolved, with no device, host-state, scheduling or driver explanation established. [Earlier baseline Field Hall](validation/v1.9/baseline-field-hall.json) · [Earlier final Field Hall](validation/v1.9/motion-final-field.json).

The earlier bounded copied-app A/B keeps the executable, complete city and renderer identical while changing only the denoiser's history cap. At 640 × 400, sixteen frames, eight current samples and 64-sample references, Field Hall records **135.207 ms with the cap versus 134.444 ms without** (0.57% difference). The flight records **47.239 ms with versus 48.858 ms without**. These observations do not support the cap as the major slowdown. The uncapped flight's image-quality failure is preserved as an ablation result. The jobs ran serially while the Mac was locked; a CPU build may have overlapped the final uncapped flight, so its overall wall time is not a clean comparison. [A/B inputs, results and limits](validation/v1.9/performance-ablation/README.md).

During that earlier A/B, initial pipeline/acceleration-structure setup took **25–38 seconds before temporal frames ran**. A subsequent scoped `ProcessInfo.beginActivity(.userInitiated)` during CLI work did not remove that slowdown: Field Hall remained **145.478 ms**, with **24.20 seconds** of initial setup, while passing the shorter image/temporal checks. Those earlier observations and their unresolved attribution are preserved alongside the later matched results above. No native FPS claim follows from these timings. [Earlier activity experiment and conclusion](validation/v1.9/performance-ablation/conclusion.json).

The paired-presentation GPU fixture verifies **55,296 accumulation channels** at 1/8/32 samples with zero maximum quantization difference, one trace budget and one traffic update per frame. The traffic fixture passes **13,944 mapped placements** and actual BLAS/TLAS build/refit, deterministic pause/rewind, moving headlight illumination and local reactive-history checks; two queued updates retain separate immutable transforms. The lighting fixture verifies **16,384 bit-identical indexed/linear pixels** and detects its reversed-order negative control. These are bounded GPU/CPU fixtures, separate from native UI interaction. [Pairing](validation/v1.9/paired-motion-validation.txt) · [Traffic](validation/v1.9/traffic-validation.txt) · [Indexed lighting](validation/v1.9/indexed-lighting-validation.txt).

## Version 1.9 package and review scope

The final production build completed in **61.76 seconds**. The locally signed **1.9.0 / build 10** app is a thin native **arm64** executable and passes strict, deep code-signature verification. Its executable SHA-256 is `fce35def02427bfbf19582dcae6aa1103f1bed7bc4a6d882e6bdf38e6496f7de`. All sixteen bundled engine resources, including Hyde Park and both current shaders, plus the app icon byte-match source. The release manifest inventories 178 source/test/script/asset/launcher files and all twenty package files; it is a file-integrity audit, not runtime proof. [Build log](validation/v1.9/build-validation.txt) · [Release manifest](validation/v1.9/release-manifest.json).

Only two source files changed after the twelve-case GPU batch: the About credit now names the Robie House–Wrigley Field extent, and four lines in the CLI `do` block begin a user-initiated activity and end it with `defer`. Reversing precisely those edits in memory reproduces the full tested file hashes. The other **176 inventoried files**, including **66 runtime/resource files** and all model, route and shader inputs, remain byte- and mode-identical. The activity changes process scheduling policy; source equivalence does not claim timing equivalence. [Preserved tested manifest](validation/v1.9/pre-final-package-manifest.json) · [Exact production comparison](validation/v1.9/production-input-equivalence.json).

A copy of the final `fce35…` app passes both **Robie/shared Chicago and Paris self-tests with repository reads explicitly denied by an OS sandbox**, at 640 × 400 and sixteen samples. The Chicago test reports **45,150,868 static triangles, 408 materials, 4,003 lights, 168 traffic vehicles / 161,812 moving triangles**, and both day/night light indices enabled. Its reported Metal allocation is **8,221.30 MiB**; Paris reports **8,645,884 triangles and 1,605.41 MiB**. These are scoped self-test allocations, not whole-process memory or sustained performance measurements. [Final standalone package checks](validation/v1.9/release-standalone-package.json) · [Earlier tested package checks](validation/v1.9/standalone-package.json).

The recorded still reviews cover all seven closer house views by day and night and selected connecting-flight samples. Day house images precede the two small exterior night fixtures; the post-polish night gallery has exact source hashes. The house roof/cantilevers, transmitting art glass, interpreted interiors, harbor arrangement and continuous shoreline/arrival are visible in those samples. Simplified neighborhood façades, park canopy and terrain, representative boat occupancy, dark unlit areas and residual grain remain explicit limits. Still-image inspection does not establish continuous animation quality. [House still review](validation/v1.9/robie-visual-review.json) · [Flight samples](validation/v1.9/hyde-park-flight-visual-review.json).

## Version 1.9 demonstration recordings

Two completed daytime recordings use the final `fce35…` package at **1280 × 720, 24 FPS and eight samples per frame**, encoded as silent H.264. The Robie House day tour contains all eight views in **96 seconds / 2,304 frames**, compressing each full route into a twelve-second chapter. The separate McCormick Place–Robie House flight retains its ordinary **360-second / 8,640-frame** timeline. The app retains all seven two-minute house studies and the full six-minute flight with independent pace controls. [Day tour media proof](validation/v1.9/robie-house-day-tour-media.json) · [Full flight media proof](validation/v1.9/mccormick-place-to-robie-house-media.json) · [Local recording links](DEMO.md#version-19-demonstration-artifacts).

All **10,944 frames** pass complete decoding, and every presentation timestamp follows the 1/24-second cadence within 0.000001 seconds. Visual review separately inspected **eight day-tour chapter midpoints and ten flight samples**, covering the harbor, shoreline, Promontory Point, Hyde Park approach and house arrival. No blocking spatial/content defect was observed in those sampled frames. This is full automated decode/timing validation plus sampled spatial/composition review, not visual inspection of every frame or a guarantee of smooth, noiseless motion. Residual roof/interior/glass grain, isolated bright speckles and simplified terrain, canopy and context façades remain documented. No nighttime movie was reviewed. [Day sample review](validation/v1.9/robie-day-video-review.json) · [Flight sample review](validation/v1.9/robie-flight-video-review.json).

Native operation of the final app verified a randomized opening at North Side view 7, Space pause, selecting view 8 while paused and continuing the demo, the next button crossing to Robie House view 1, and Down wrapping Robie House’s last view to Willis Tower’s first. The previous button returned to Robie House view 8; inspected screenshots confirmed night at Willis and daytime after the reverse wrap. Selecting Museum Campus from the location menu kept the demo playing at its first view. The selected eighth card remained visible. These are sampled transitions, not a complete demo pass. [Native control record](validation/v1.9/native-ui.json).

The Mac relocked during the next check. Additional current-release focus/orbit, titlebar movement, resize/fullscreen, L cycling and Paris roundtrip checks were not completed. Earlier native records and current CPU geometry/control checks retain their separate scopes. Exact camera matrices and window origins were not recorded, and sampled HUD values are not a sustained FPS benchmark. [Export commands, hashes and timing scope](validation/v1.9/media-exports.json).

## Version 1.8 Chicago demo and object focus

This release adds a complete **40-route Chicago demo**, visible-geometry object selection and orbit/zoom controls, and camera/scene-clock suspension during native window movement and live resizing. The demo traverses all five Chicago destinations in the resident city, lasts **66 minutes 36 seconds at 1×**, preserves starting lighting and pace, and alternates day/night after each full pass. [Controls](DEMO.md) · [Focus behavior and limits](FOCUS.md).

The completed CPU reports cover the following scopes:

| Check | Recorded result and scope |
| --- | --- |
| [Playback and camera clearance](validation/v1.8/playback-validation.txt) | **57,929 checks, no failures**, including 412 added demo checks and the established clearance/support samples across all 49 routes. Demo coverage includes ordering, complete passes, elapsed-time carry, invalid/very large elapsed values, pause/stop, manual exits and route-local transport. |
| [Focus unit checks](validation/v1.8/focus-navigation-unit.txt) | **1,578 checks, PASS**: screen rays, nearest represented surface/occlusion, focus toggling, bounded orbit/zoom, mapped identities, and separate dense picking versus navigation queries. |
| [Actual Chicago focus integration](validation/v1.8/focus-navigation-integration.json) | **PASS**: Adler, Willis and Cloud Gate picks, sky miss, all **171,264** Bean shell triangles retained in the optional detail index, and the original **30,019,005** navigation triangles retained. The complete world still has **39,792,802** static triangles. |
| [Viewport pointer ownership](validation/v1.8/viewport-input-validation.json) | **187 checks, no failures**: click/drag separation, rejected nonviewport/orphan events, button ownership, window translation, invalid values and lifecycle cancellation. Its negative control emits 12 unwanted look events; the owned-gesture path emits zero. This is a CPU contract check, not AppKit event-delivery proof. |

The playback report was produced before the later picking-only collision supplement; its playback implementation and test file still match the completed run. Subsequent focus fixtures cover the supplement independently. The original navigation BVH builder and movement/floor/ray query bodies remain byte-identical to 1.7, and unit fixtures compare retained navigation triangle order and positions. Collection-time source hashes and exact copied-report hashes are retained in the [evidence index](validation/v1.8/evidence-index.json).

The first completed 1.8 app build passed both packaged-world self-tests at **640 × 400 / 16 samples**, covering geometry, Swift/Metal ABI, hardware dispatch, progressive accumulation, viewport checks, navigation and OBJ/MTL import. Chicago retains **39,792,802 static triangles, 355 materials, 3,222 static lights and 168 traffic vehicles / 161,812 moving triangles**; its reported Metal allocation is **7,255.92 MiB**. Paris retains **8,645,884 triangles** with **1,605.41 MiB** reported allocations. These are scoped self-test allocations, not whole-process memory or sustained frame-rate measurements. This release's records do not repeat the earlier repository-denied standalone sandbox check. [Build](validation/v1.8/build-validation.txt) · [Chicago](validation/v1.8/northside-self-test.json) · [Paris](validation/v1.8/paris-self-test.json).

**The 33 motion cases were not rerun for 1.8.** A byte comparison against commit `1826df1adce3ce03165cac6c3ef2b3adbe323fa8` confirms **55 existing runtime/resource files are unchanged**, including all **14 resource files**, the renderer and shaders, static geometry, traffic implementation, camera tracks and route definitions. The Paris route-definition section inside the modified playback file is also byte-identical. Changes to existing runtime files are confined to the app UI, controller, playback state machine and optional picking support; two new helpers implement focus and pointer ownership. The [exact comparison](validation/v1.8/rendering-input-equivalence.json) supports retaining the previous motion measurements as historical evidence. It does not establish motion quality for every new manual orbit or certify native clock handling. The 1.7 recordings below remain earlier-version artifacts.

The bounded [controller review](validation/v1.8/controller-review.json) found and verified source fixes for restoring rendered lighting when starting a demo from Paris and retaining the viewport responder after Escape clears focus. It found no remaining substantive issue within that review scope. The [native control record](validation/v1.8/native-controls.json) separately verifies sampled demo transitions across all five destinations, the Wrigley-to-Willis night wrap, paused lighting changes, and Paris-to-Chicago startup retaining night and 2× pace. Actual Adler clicks, orbit drags, zoom in both directions, click-to-release and Escape followed by Space were exercised. Native resize and fullscreen retained focus; titlebar dragging left the window-relative camera framing visibly unchanged. Exact window-origin and camera-matrix telemetry were not captured. This was sampled boundary inspection, largely by seeking near route endpoints, rather than watching a complete 66-minute pass; route 24 was not directly inspected. Physical trackpad hardware was not separately tested. Glass is a picking surface; optical transmission, reflected objects, tiny decorations and moving traffic are not independent picking targets. Focus orbit clearance does not provide collision avoidance against neighboring buildings.

The final package was rebuilt after three small refinements: a glass background behind the demo counter, a full-pass duration label, and finer precise-trackpad zoom scaling. Its native smoke check reconfirmed startup, C, the new labels, Adler focus/orbit/zoom and Escape-to-C. The GPU self-tests above used the preceding 1.8 package; renderer, shaders, resources, focus helpers and playback remained unchanged for this final UI/input refinement. [Final build](validation/v1.8/final-build-validation.txt) · [Final package and source inventory](validation/v1.8/final-package-input.json).
