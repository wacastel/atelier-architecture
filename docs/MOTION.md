...
## Chicago interior grain and traffic guide correction (2.4)

The September 2026 reports of grain in the Cultural Center stair and Willis Catalog prompted separate tests of reconstruction history and lighting variance. The sampled stair retained about 29.5 accepted history frames across its ordinary surfaces at the end of a two-second idle sequence. It had no fully reactive traffic pixels; the nearest mapped moving-traffic lane is about 660 m from the Cultural Center center. Marble uses a plain, roughness-0.38 material. These measurements do not support blaming the staircase grain on missing motion history, traffic, or a noisy procedural marble texture.

A real, separate traffic error was corrected. The previous primary-surface pass marked everything within a moving vehicle lamp's support radius—65 m for a headlamp—as fully reactive even in daylight, although the daylight tracer does not evaluate those lamps. The corrected pass checks the lighting mode, actual light range/cone/surface-facing contribution and visibility. An opaque intervening wall therefore does not unnecessarily disable a room's reconstruction. Nearby moving sunlight shadows and reflected moving geometry retain explicit detection. Changing illumination on stationary, nonemissive, nonmetallic rough surfaces uses the existing `+0.25` current-lighting guide: it rejects old illumination while retaining compatible current-frame spatial samples. Moving geometry and sharp moving reflections retain `+0.875` coverage. This is a local primary-surface policy, not exhaustive tracking of all indirect moving shadows or reflections.

Run `swift scripts/validate-traffic-guides.swift` for the small production-kernel regression. Its 13 actual two-instance acceleration-structure cases cover inactive daytime lamps, lit and occluded nighttime surfaces, cone/range/backface exclusions, light through glass, a moving sunlight shadow and its departure, visible traffic geometry, and a moving vehicle in a polished reflection. All pass. The original shader reproduces the incorrect daylight `+0.875` guide in an explicit negative control. [The guide report](validation/v2.4/path-noise/traffic-guides.json) records the shader hashes and individual results.

`MotionValidation.swift` now has named Cultural exterior, Washington entrance, stair and Preston Hall idle cases, plus a Willis Catalog idle case. The new cases follow the actual idle poses with an advancing scene clock rather than using a different destination's fallback routes. Bounded diagnostic measurements from the pre-change scene are:

| Idle view | Raw image RMSE | Reconstructed image RMSE | Raw temporal residual | Reconstructed temporal residual |
|---|---:|---:|---:|---:|
| Cultural mosaic stair | 0.170382 | 0.108560 | 0.231227 | 0.137603 |
| Willis Catalog | 0.101714 | 0.041495 | 0.144995 | 0.053895 |

These are 48 frames at 256 × 160, four traced samples per frame, and independent 64-sample references over 1.96 seconds. Raw and reconstructed presentations share each frame's trace accumulation. References still contain visible sampling grain; the figures are not measurements at the full native window resolution. Catalog's through-glass surfaces intentionally have no temporal history because transmitted and reflected images have different motion. Their current-frame spatial reconstruction remains enabled.

A stronger variance-dependent spatial filter was tested and rejected. It reduced RMS error by 28.6% in a controlled noisy-history fixture and preserved the tested narrow illumination lines, material boundaries, mirrors and glass. On the actual Cultural stair, however, it improved image RMSE by only 0.7% and temporal residual by only 0.24%. That result did not justify additional filtering complexity. The release keeps the previous denoiser byte-for-byte. The experiment was informed by [the original SVGF research](https://research.nvidia.com/labs/rtr/publication/schied2017spatiotemporal/), but it was not a full implementation of its temporal variance estimator. [The investigation record](validation/v2.4/path-noise/review.json) preserves the baseline, rejected source, measurements and limitations.

A diagnostic that removed secondary bounces substantially reduced the staircase's grain, while also changing and darkening its illumination. It is evidence that difficult indirect lighting matters here, not an acceptable replacement for the original path-traced lighting. The separate [Direct Ray Tracing mode](DIRECT-RAY-TRACING.md) offers deterministic visibility and reflection rays with an explicit ambient-light approximation; it has different lighting fidelity from Path Tracing.

Indirect-only luminance clamping remains future work. [Blender's Cycles documentation](https://docs.blender.org/manual/en/4.1/render/cycles/render_settings/light_paths.html) describes its variance-versus-brightness tradeoff, and [PBRT discusses the difficult indirect paths and sampling alternatives](https://pbr-book.org/4ed/Light_Transport_I_Surface_Reflection/A_Better_Path_Tracer). A suitable experiment would keep primary emission, direct illumination and primary glossy/glass reflection chains separate, then limit only contributions following a rough or diffuse scatter. It must retain the unclamped baseline as a control and measure brightness loss as well as noise. No new indirect clamp, sample budget, blur radius or path-tracing material roughness change is included in this release.
