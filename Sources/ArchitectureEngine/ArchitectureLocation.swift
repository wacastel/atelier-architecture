...
enum ArchitectureLocation: String, CaseIterable, Identifiable {
    ...
    var name: String {
        switch self { case .paris: return "Eiffel Tower"; case .chicago: return "Willis Tower"; case .millennium: return "Millennium Park"; case .lakefront: return "Chicago Lakefront"; case .campus: return "Museum Campus"; case .northside: return "Chicago North Side" }
    }
    var shortName: String { self == .paris ? "Paris" : "Chicago" }
    var subtitle: String {
        switch self { case .paris: return "PARIS, FRANCE   ·   1889"; case .chicago: return "CHICAGO, ILLINOIS   ·   1973"; case .millennium: return "CHICAGO, ILLINOIS   ·   ART & LANDSCAPE"; case .lakefront: return "MICHIGAN AVENUE   ·   PARKS & HARBORS"; case .campus: return "MUSEUMS   ·   STARS   ·   THE SOUTH LAKEFRONT"; case .northside: return "OLD TOWN   ·   LINCOLN PARK   ·   WRIGLEYVILLE" }
    }
    var number: String { self == .paris ? "01" : self == .chicago ? "02" : self == .millennium ? "03" : self == .lakefront ? "04" : self == .campus ? "05" : "06" }
    /// Chicago locations share one physical scene and acceleration structure.
    var world: String { self == .paris ? "paris" : "chicago" }
    var stops: [TourStop] {
        switch self { case .paris: return EiffelScene.stops; case .chicago: return WillisScene.stops; case .millennium: return MillenniumScene.stops; case .lakefront: return LakefrontScene.stops; case .campus: return MuseumCampusScene.stops; case .northside: return NorthSideScene.stops }
    }
    func duration(view: Int) -> Double {
        switch self { case .paris: return EiffelWalkthrough.duration; case .chicago: return WillisWalkthrough.duration; case .millennium: return MillenniumWalkthrough.duration(view: view); case .lakefront: return LakefrontWalkthrough.duration(view: view); case .campus: return MuseumCampusWalkthrough.duration(view: view); case .northside: return NorthSideWalkthrough.duration(view: view) }
    }
    var walkingViews: Set<Int> {
        switch self { case .paris: return [1,3,4,5,6]; case .chicago: return WillisWalkthrough.walkingViews; case .millennium: return MillenniumWalkthrough.walkingViews; case .lakefront: return LakefrontWalkthrough.walkingViews; case .campus: return MuseumCampusWalkthrough.walkingViews; case .northside: return NorthSideWalkthrough.walkingViews }
    }
