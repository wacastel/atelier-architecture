...
def main() -> None:
    ...
    expected_keys = ["paris", "chicago", "millennium", "lakefront", "campus", "northside", "robie", "skyline"]
    expect([t["location"] for t in manifest["tracks"]] == expected_keys, "Incomplete or reordered soundtrack catalog")
    expect(len({t["sha256"] for t in manifest["tracks"]}) == 8, "An audio asset is reused")
    expect(len({t["compositionEventSHA256"] for t in manifest["tracks"]}) == 8, "Compositions share an identical event sequence")
    reports = []
