...
mccormick_tmp="$(mktemp -d "${TMPDIR:-/tmp}/atelier-mccormick.XXXXXX")"
trap 'rm -rf -- "$mccormick_tmp"' EXIT
...
xcrun swiftc -O -whole-module-optimization \
  ...
  "$project_dir/Tests/McCormick/main.swift" \
  -o "$mccormick_tmp/validate-mccormick"
(cd "$project_dir" && "$mccormick_tmp/validate-mccormick")
