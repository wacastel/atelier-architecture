...
benchmark_tmp="$(mktemp -d "${TMPDIR:-/tmp}/atelier-skyline.XXXXXX")"
...
scene_sources+=(
  ...
  "$project_dir/Tests/SkylineRendering/main.swift"
)
printf '%s\n' "${scene_sources[@]}" "$project_dir/scripts/validate-skyline-rendering.sh" > "$benchmark_tmp/inputs.txt"
xcrun swiftc -O -whole-module-optimization "${scene_sources[@]}" -o "$benchmark_tmp/skyline-rendering"
(cd "$project_dir" && ATELIER_BENCHMARK_INPUTS="$benchmark_tmp/inputs.txt" "$benchmark_tmp/skyline-rendering")
